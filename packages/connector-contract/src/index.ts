...
export const experimentalTailV1ConversationHistory = (
  ...
  for (let index = source.length - 1; index >= 0; index -= 1) {
    ...
    const candidate: CanonicalConversationItem = {
      role: item.role,
      content: item.content.slice(0, EXPERIMENTAL_TAIL_V1_ITEM_CHAR_LIMIT),
    };
