...
describe('room event contract', () => {
  it('accepts a valid server event', () => {
    ...
    expect(isServerRoomEvent({id:'event-intervention',sequence:3,type:'run.intervention.updated',payload:{runId:'run-1',intervention:{id:'c226f522-d864-4f1c-a53f-25d22dc9109f',text:'Focus on the API',status:'pending',precedingText:'Earlier',author:{profileId:'local-user',displayName:'User',handle:'user'},createdAt:'2026-07-20T12:01:00.000Z'}}})).toBe(true);
    expect(isServerRoomEvent({id:'event-legacy-intervention',sequence:4,type:'run.intervention.updated',payload:{runId:'run-1',intervention:{id:'777f7444-e6a9-4e85-818f-20d536876ff7',text:'Legacy instruction',status:'applied',supersededText:'Earlier'}}})).toBe(true);
