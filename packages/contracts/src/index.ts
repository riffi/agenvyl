...
export type RuntimeFeatures = { plan_mode: boolean; preview_origin: string };
...
export type RoomExecutionState = { plan: RoomPlanState };
...
export type SetupHarnessCandidate={type:HarnessType;label:string;cli:{found:boolean;command:string;version?:string;compatible?:boolean};endpoint?:{url:string;reachable:boolean};safeToSelect:boolean;supportsManagedServer:boolean;auth?:HarnessAuthentication;requiresConfirmation?:'claude_oauth'|'cursor_experimental';warning?:string};
export type SetupHarnessInstance={id:string;type:HarnessType;enabled:boolean;endpoint?:string;managed?:boolean;externalDirectoryRoots?:string[];allowSubscriptionOAuth?:boolean};
export type SetupRuntimeInstance=SetupHarnessInstance&{status:'healthy'|'degraded'|'unavailable'|'disabled';error?:{code:string;message:string}};
export type SetupState={completed:boolean;locale:'en'|'ru';workspaceRoot:string;firstRoomId?:string;instances:SetupRuntimeInstance[];candidates:SetupHarnessCandidate[];discoveryCache:HarnessCacheMetadata};
...
export type RoomWorkspace={path:string;current_snapshot_id:string;materialization_status:'pending'|'ready'|'failed';entries:WorkspaceEntry[]};
...
export type ServerRoomEvent =
  ...
  | Envelope<'run.workspace.finalized', {runId:string;workspaceResult:RunWorkspaceResult}>
...
export function isServerRoomEvent(value: unknown): value is ServerRoomEvent {
  ...
  switch (value.type) {
    ...
    case 'run.workspace.finalized': case 'run.workspace.publish.updated': return typeof payload.runId==='string'&&isRunWorkspaceResult(payload.workspaceResult);
