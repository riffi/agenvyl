...
describe.skipIf(process.platform !== 'win32')('PowerShell installer integration', () => {
  ...
  it('installs a locally built archive through the production installer', async () => {
    ...
    const root = await fixture(currentVersion); roots.push(root.path);
    ...
    await expect(stat(join(root.installRoot, currentVersion, 'manifest.json'))).resolves.toBeTruthy();
  }, 15_000);
});

async function fixture(version = '0.1.0') {
  const path = await mkdtemp(join(tmpdir(), 'agenvyl-windows-installer-'));
  const downloads = join(path, 'downloads'), bundleParent = join(path, 'bundle'), bundleName = `agenvyl-${version}-windows-x64`, bundleRoot = join(bundleParent, bundleName);
  ...
  await writeFile(join(bundleRoot, 'manifest.json'), JSON.stringify({ name: 'agenvyl-portable-runtime', version }));
  await writeFile(join(bundleRoot, 'bin', 'agenvyl.cmd'), '@echo off\r\necho %* >> "%AGENVYL_INIT_LOG%"\r\nexit /b 0\r\n');
  const filename = `${bundleName}.zip`, archive = join(downloads, filename);
  const zip = spawnSync(join(process.env.SystemRoot ?? 'C:\\Windows', 'System32', 'tar.exe'), ['-a', '-cf', archive, bundleName], { cwd: bundleParent, encoding: 'utf8' });
