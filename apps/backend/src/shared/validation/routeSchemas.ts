...
export const runInterventionResponseSchema=objectSchema({intervention_id:{type:'string',format:'uuid'},status:{type:'string',const:'pending'}},['intervention_id','status']);
...
const runInterventionResponseItemSchema=objectSchema({id:{type:'string'},text:{type:'string'},status:{type:'string',enum:['pending','applied','failed']},supersededText:{type:'string'},error:{type:'string'}},['id','text','status']);
...
const workspaceAttachmentResponseSchema=objectSchema({version_id:{type:'string'},entry_id:{type:'string'},snapshot_id:{type:'string'},name:{type:'string'},path:{type:'string'},size:{type:'integer',minimum:0},mime_type:{type:'string'},url:{type:'string'},preview_url:{type:'string'}},['version_id','name','path','size','mime_type','url','preview_url']);
