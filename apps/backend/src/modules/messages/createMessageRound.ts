...
export class CreateMessageRound {
  constructor(private readonly dependencies: { personas: PersonaRepository; rooms:RoomRepository; messages: MessageRepository; events: RoomEventService; harnesses:{catalog():Promise<{instances:Array<{id:string;type:string;status:string;models:ConnectorCatalogModel[];controls:ConnectorExecutionControls}>}>}; activeRuns: ActiveRunRegistry; runExecutor: RunExecutor;roomWorkspace?:RoomWorkspaceService }) {}
  ...
  async execute(command: { roomId: string; text?: string; targets?: string[]; messageId?: string;attachmentVersionIds?:string[];correlationId?:string }) {
    ...
    if (targets.length) {
      const state=await this.dependencies.rooms.executionState(command.roomId);
      ...
      for(const persona of targets){
        ...
        if(persona.permission_profile_id&&!instance.controls.permissionProfiles.some(item=>item.id===persona.permission_profile_id))throw new AppError('unknown_permission_profile',400,'Unknown permission profile',{permissionProfileId:persona.permission_profile_id,harnessInstanceId:persona.harness_instance_id,persona:persona.handle});
        if(persona.agent_variant_id&&!instance.controls.agentVariants.some(item=>item.id===persona.agent_variant_id))throw new AppError('unknown_agent_variant',400,'Unknown agent variant',{agentVariantId:persona.agent_variant_id,harnessInstanceId:persona.harness_instance_id,persona:persona.handle});
        executionProfiles.set(persona.id,resolveExecutionProfile({state,model,controls:instance.controls,permissionProfileId:persona.permission_profile_id,agentVariantId:persona.agent_variant_id}));
    ...
    let round;try{round=await messages.createRound(command.roomId, text, targets,executionProfiles, command.messageId,command.attachmentVersionIds??[],addressedToAll);}catch(error){if(error instanceof Error&&error.message==='attachment_unavailable')throw new AppError('attachment_unavailable',409,'Attachment version is unavailable');throw error;}
    ...
    for (const item of round.runs) activeRuns.add({ id: item.id, messageId:round.message.id, roomId: command.roomId, personaVersionId: item.version.id, personaHandle: item.persona.handle, requestedModel: item.version.requested_model!, harnessInstanceId:item.version.harness_instance_id,harnessType:item.version.harness_type,modelId:item.version.model_id,executionProfile:item.executionProfile,conversationHistory: item.history,correlationId:command.correlationId, terminal: false,started:false,refreshContext:true });
