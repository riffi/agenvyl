...
export type FollowUpAnchor={runId:string;roomId:string;personaId:string;personaHandle:string;status:string;harnessInstanceId:string;harnessType:string};
export type PendingFollowUp={id:string;roomId:string;messageId:string;personaId:string;personaHandle:string;anchorRunId:string;deliveryKind:'after_response'|'apply_now';status:string;text:string;executionProfile:RunExecutionProfileSnapshot;transitionReason?:'workflow_mode_changed'};
...
export class FollowUpRepository{
  ...
  async create(input:{roomId:string;text:string;messageId:string;anchor:FollowUpAnchor;deliveryKind:'after_response'|'apply_now';resolveProfile:ProfileResolver;requireTransition?:boolean;attachmentVersionIds?:string[]}){
    return this.database.transaction(async tx=>{
      ...
      const deliveryKind=transitionReason?'after_response':input.deliveryKind;
      const now=new Date().toISOString(),pendingId=crypto.randomUUID(),route=deliveryKind==='apply_now'?'active_intervention':'agent_session';
  ...
  async claimApplyNow(roomId:string,messageId:string){
    return this.database.transaction(async tx=>{
      ...
      const now=new Date().toISOString(),delivery:MessageDelivery={route:transitionReason?'agent_session':'active_intervention',status:'dispatching',...(transitionReason?{transitionReason}:{}),agent:String(row.persona_handle),anchorRunId:String(row.anchor_run_id)};
      ...
      await tx`UPDATE room_messages SET delivery_route=${delivery.route},delivery_status='dispatching',delivery_error=NULL,delivery_updated_at=${now} WHERE id=${messageId}`;
  ...
  async requeueApplyNow(id:string,error?:string){
    return this.database.transaction(async tx=>{
      ...
      const transitionReason=row.transition_reason==='workflow_mode_changed'?'workflow_mode_changed' as const:undefined;
      const now=new Date().toISOString(),delivery:MessageDelivery={route:'agent_session',status:'queued',...(transitionReason?{transitionReason}:{}),agent:String(row.persona_handle),anchorRunId:String(row.anchor_run_id),...(error?{error}:{})};
  ...
  async prepareHandoffCancellation(id:string){
    await this.database.sql`UPDATE pending_agent_follow_ups SET delivery_kind='after_response',updated_at=now() WHERE id=${id} AND delivery_kind='apply_now' AND status='dispatching' AND transition_reason='workflow_mode_changed'`;
  }

  async recordQueuedError(id:string,error:string){
    return this.database.transaction(async tx=>{
      const[row]=await tx`SELECT p.*,m.delivery_route,m.delivery_transition_reason FROM pending_agent_follow_ups p JOIN room_messages m ON m.id=p.message_id WHERE p.id=${id} FOR UPDATE OF p,m`;
      if(!row||!['queued','dispatching'].includes(String(row.status)))return undefined;
      const transitionReason=row.delivery_transition_reason==='workflow_mode_changed'?'workflow_mode_changed' as const:undefined,now=new Date().toISOString();
      const delivery:MessageDelivery={route:'agent_session',status:'queued',...(transitionReason?{transitionReason}:{}),agent:String(row.persona_handle),anchorRunId:String(row.anchor_run_id),error};
      await tx`UPDATE room_messages SET delivery_route='agent_session',delivery_status='queued',delivery_error=${error},delivery_updated_at=${now} WHERE id=${row.message_id as string}`;
      await tx`UPDATE pending_agent_follow_ups SET delivery_kind='after_response',status='queued',claimed_at=NULL,updated_at=${now} WHERE id=${id}`;
      const event=await this.events.appendInTransaction(tx,String(row.room_id),'message.delivery.updated',{messageId:String(row.message_id),delivery},now);
      return{roomId:String(row.room_id),event,delivery};
    });
  }
  ...
  async markDelivery(id:string,status:MessageDelivery['status'],options:{route?:MessageDelivery['route'];runId?:string;error?:string;final?:boolean}={}){
    return this.database.transaction(async tx=>{
      ...
      const transitionReason=row.delivery_transition_reason==='workflow_mode_changed'?'workflow_mode_changed' as const:undefined;
      const delivery:MessageDelivery={route,status,...(transitionReason?{transitionReason}:{}),agent:String(row.persona_handle),anchorRunId:String(row.anchor_run_id),...(options.runId?{runId:options.runId}:{}),...(options.error?{error:options.error}:{})};
      const event=await this.events.appendInTransaction(tx,String(row.room_id),'message.delivery.updated',{messageId:String(row.message_id),delivery},now);
...
function anchor(row:Record<string,unknown>):FollowUpAnchor{return{runId:String(row.id),roomId:String(row.room_id),personaId:String(row.persona_id),personaHandle:String(row.persona_handle),status:String(row.status),harnessInstanceId:String(row.harness_instance_id),harnessType:String(row.harness_type)};}
function pending(row:Record<string,unknown>):PendingFollowUp{return{id:String(row.id),roomId:String(row.room_id),messageId:String(row.message_id),personaId:String(row.persona_id),personaHandle:String(row.persona_handle),anchorRunId:String(row.anchor_run_id),deliveryKind:row.delivery_kind as PendingFollowUp['deliveryKind'],status:String(row.status),text:String(row.text),executionProfile:row.execution_profile_snapshot as RunExecutionProfileSnapshot,...(row.transition_reason==='workflow_mode_changed'?{transitionReason:'workflow_mode_changed' as const}:{})};}
