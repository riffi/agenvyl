...
export class RoomsService{
  ...
  async approvePlan(roomId:string,versionId:string){const state=await this.rooms.approvePlan(roomId,versionId);if(!state)throw new AppError('invalid_approved_plan',409,'Only the current plan.md version can be approved');await this.events?.emit(roomId,'room.plan.approval.updated',{approved:state.plan.approved});return state;}
  async clearApprovedPlan(roomId:string){const state=await this.rooms.clearApprovedPlan(roomId);if(!state)throw new AppError('room_not_found',404,'Room not found');await this.events?.emit(roomId,'room.plan.approval.updated',{approved:null});return state;}
}
