...
export class PersonasService{
  ...
  async delete(id:string){const result=await this.personas.delete(id);if(result.status==='not_found')throw new AppError('not_found',404,'Agent not found');if(result.status==='in_use')throw new AppError('persona_in_use',409,'The agent has already been used in runs and cannot be deleted.',{dependencies:result.dependencies});}
