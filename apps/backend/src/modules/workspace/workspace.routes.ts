...
import {registerWorkspaceRestoreRoutes} from './workspaceRestore.routes.js';
import {runPreviewAssetParamsSchema,runPreviewParamsSchema} from '../../shared/validation/routeSchemas.js';

export async function registerWorkspaceRoutes(app:FastifyInstance,workspace:RoomWorkspaceService){
  await registerWorkspaceRestoreRoutes(app,workspace.restores);
  ...
  app.get<{Params:{roomId:string;runId:string}}>('/api/v1/rooms/:roomId/runs/:runId/preview',{schema:{params:runPreviewParamsSchema}},runPreview);
  app.get<{Params:{roomId:string;runId:string}}>('/api/v1/rooms/:roomId/runs/:runId/preview/',{schema:{params:runPreviewParamsSchema}},runPreview);
...
const htmlCsp="default-src 'self' http: https: data: blob:; img-src 'self' http: https: data: blob:; style-src 'self' http: https: 'unsafe-inline'; script-src 'self' http: https: 'unsafe-inline' 'wasm-unsafe-eval' blob:; connect-src 'self' http: https: ws: wss:; font-src 'self' http: https: data:; media-src 'self' http: https: data: blob:; frame-src http: https:; worker-src 'self' http: https: blob:; object-src 'none'";
...
const sendImmutablePreview=async(reply:FastifyReply,file:Awaited<ReturnType<RoomWorkspaceService['resolveRunPreview']>>,base:string,ifNoneMatch?:string)=>{
  ...
  if(file.contentType!=='text/html')return response.send(file.data);
  const source=file.data.toString('utf8'),tag=`<base href="${base}">`;
