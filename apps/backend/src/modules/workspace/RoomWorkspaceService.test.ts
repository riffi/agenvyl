...
function fixture(root:string,max=1024,activeValues:()=>any[]=()=>[]){
  ...
  const service=new RoomWorkspaceService({exists:vi.fn().mockResolvedValue(true),hasActivePlanRun:vi.fn().mockImplementation(async()=>activeValues().some(run=>run.executionProfile?.workflowMode==='plan'&&!run.terminal))} as never,repository as never,{emit:vi.fn()} as never,{values:activeValues} as never,root,'/host/workspaces',max);
