...
export type PreviewBundleRow={id:string;runId:string;roomId:string;sourceHead?:string;entrypoint:string;sourceManifestSha256:string;bundleSha256?:string;bundleSize?:number;uncompressedSize:number;fileCount:number;status:'capturing'|'ready'|'failed';error?:string;createdAt:string;updatedAt:string};
...
export class WorkspaceRepository{
  ...
  async applyRunArtifactVisibility(runId:string,policy:{visibility:(path:string,kind?:'file'|'directory')=>'project'|'hidden'}){const rows=await this.database.sql`SELECT ra.version_id,v.path FROM run_artifacts ra JOIN workspace_versions v ON v.id=ra.version_id WHERE ra.run_id=${runId}`;for(const row of rows)await this.database.sql`UPDATE run_artifacts SET visibility=${policy.visibility(text(row.path))} WHERE run_id=${runId} AND version_id=${text(row.version_id)}`;}
  async beginPreviewBundle(input:{roomId:string;runId:string;sourceHead:string;entrypoint:string;sourceManifestSha256:string;uncompressedSize:number;fileCount:number}){
    ...
    const row=(await this.database.sql`INSERT INTO preview_bundles(id,run_id,room_id,source_head,entrypoint,source_manifest_sha256,uncompressed_size,file_count,status,created_at,updated_at)
      VALUES(${id},${input.runId},${input.roomId},${input.sourceHead},${input.entrypoint},${input.sourceManifestSha256},${input.uncompressedSize},${input.fileCount},'capturing',${now},${now})
      ON CONFLICT(run_id) DO UPDATE SET source_head=EXCLUDED.source_head,entrypoint=EXCLUDED.entrypoint,source_manifest_sha256=EXCLUDED.source_manifest_sha256,uncompressed_size=EXCLUDED.uncompressed_size,file_count=EXCLUDED.file_count,status='capturing',bundle_sha256=NULL,bundle_size=NULL,error=NULL,updated_at=EXCLUDED.updated_at RETURNING *`)[0];
  ...
  async previewBundles(roomId:string):Promise<PreviewBundleHistoryRow[]>{return(await this.database.sql`SELECT pb.*,r.persona_handle,r.status run_status,r.created_at run_created_at FROM preview_bundles pb JOIN agent_runs r ON r.id=pb.run_id WHERE pb.room_id=${roomId} AND pb.status='ready' ORDER BY r.updated_at DESC,pb.created_at DESC,pb.id DESC`).map(toPreviewBundleHistoryRow);}
  ...
  async artifactProjections(runIds:string[],db:QueryContext=this.database.sql){
    ...
    for(const row of await db`SELECT ra.run_id,ra.change,ra.attribution,v.* FROM run_artifacts ra JOIN workspace_versions v ON v.id=ra.version_id WHERE ra.run_id=ANY(${runIds}) AND ra.visibility='project' ORDER BY ra.created_at,v.path`){
      const runId=text(row.run_id);
      ensure(runId).artifacts.push({...toAttachment(toVersionRow(row)),change:text(row.change) as RunArtifact['change'],attribution:text(row.attribution) as RunArtifact['attribution']});
    }
  ...
  async runEmbeds(runIds:string[],db:QueryContext=this.database.sql){if(!runIds.length)return new Map<string,RunEmbed[]>();const rows=await db`SELECT re.run_id,re.position,re.kind,re.path embed_path,re.error,v.id version_id,v.entry_id,v.path version_path,v.size,v.mime_type,v.room_id FROM run_embeds re LEFT JOIN workspace_versions v ON v.id=re.version_id WHERE re.run_id=ANY(${runIds}) ORDER BY re.run_id,re.position`;const result=new Map<string,RunEmbed[]>();for(const row of rows){const id=text(row.run_id),items=result.get(id)??[],error=row.error?text(row.error) as NonNullable<RunEmbed['error']>:undefined;let attachment:WorkspaceAttachment|undefined;if(row.version_id){const roomId=text(row.room_id),versionId=text(row.version_id),versionPath=text(row.version_path);attachment={version_id:versionId,...(row.entry_id?{entry_id:text(row.entry_id)}:{}),path:versionPath,name:versionPath.split('/').pop()??versionPath,size:number(row.size),mime_type:text(row.mime_type),url:versionUrl(roomId,versionId),preview_url:`${versionUrl(roomId,versionId)}/preview`};}items.push({kind:'image',path:text(row.embed_path),status:attachment?'resolved':'error',...(attachment?{attachment}:{error:error??'not_found'})});result.set(id,items);}return result;}
...
export function toAttachment(value:VersionRow):WorkspaceAttachment{return{version_id:value.id,...(value.entry_id?{entry_id:value.entry_id}:{}),path:value.path,name:value.path.split('/').pop()??value.path,size:value.size,mime_type:value.mime_type,url:versionUrl(value.room_id,value.id),preview_url:`${versionUrl(value.room_id,value.id)}/preview`};}
export function previewAttachment(value:PreviewBundleRow):WorkspaceAttachment{return{version_id:value.id,path:value.entrypoint,name:value.entrypoint.split('/').pop()??value.entrypoint,size:value.bundleSize??value.uncompressedSize,mime_type:'text/html',url:runPreviewUrl(value.roomId,value.runId),preview_url:runPreviewUrl(value.roomId,value.runId)};}
...
const toPreviewBundleRow=(row:Record<string,unknown>):PreviewBundleRow=>({id:text(row.id),runId:text(row.run_id),roomId:text(row.room_id),...(row.source_head?{sourceHead:text(row.source_head)}:{}),entrypoint:text(row.entrypoint),sourceManifestSha256:text(row.source_manifest_sha256),...(row.bundle_sha256?{bundleSha256:text(row.bundle_sha256)}:{}),...(row.bundle_size!=null?{bundleSize:number(row.bundle_size)}:{}),uncompressedSize:number(row.uncompressed_size),fileCount:number(row.file_count),status:text(row.status) as PreviewBundleRow['status'],...(row.error?{error:text(row.error)}:{}),createdAt:timestamp(row.created_at),updatedAt:timestamp(row.updated_at)});
const toPreviewBundleHistoryRow=(row:Record<string,unknown>):PreviewBundleHistoryRow=>({...toPreviewBundleRow(row),agent:text(row.persona_handle),runStatus:text(row.run_status),runCreatedAt:timestamp(row.run_created_at)});
