...
export class WorkspaceRepository{
  ...
  async versions(roomId:string,entryId:string){return(await this.database.sql`SELECT v.* FROM workspace_versions v JOIN workspace_entries e ON e.room_id=v.room_id AND e.path=v.path WHERE v.room_id=${roomId} AND e.id=${entryId} ORDER BY v.created_at DESC`).map(toVersionRow);}
