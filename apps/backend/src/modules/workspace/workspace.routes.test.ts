...
describe('run workspace routes',()=>{
  ...
  it('serves HTML and assets read directly from an immutable ZIP bundle',async()=>{
    const resolveRunPreview=vi.fn(async(_roomId:string,_runId:string,asset='')=>{
      ...
      return{data,contentType:script?'text/javascript':'text/html',snapshotId:'result',version:{id:'preview-id',path:script?'assets/app.js':'index.html',size:data.length,mime_type:script?'text/javascript':'text/html',sha256:script?'bundle-script':'bundle-html',source:'agent' as const,run_ids:['run'],created_at:'2026-08-10T00:00:00.000Z',url:'/preview',preview_url:'/preview'}};
    }),app=Fastify({logger:false});
    await registerErrorHandler(app);
    await registerWorkspaceRoutes(app,{maxFileBytes:1024,resolveRunPreview} as unknown as RoomWorkspaceService);
