...
describe('persisted workspace embeds',()=>{
  it('pins the resolved image version and keeps snapshot metadata out of natural-language history',async()=>{
    ...
    try{
      ...
      const persona=(await repositories.personas.find('persona-architect'))!,round=await repositories.messages.createRound('demo-room','show it',[persona]);
      ...
      const service=new CreateMessageRound({personas:repositories.personas,messages:repositories.messages,events,harnesses:{catalog:vi.fn().mockResolvedValue({instances:[{id:'local-hermes',type:'hermes',status:'healthy',models:[{id:'sol',label:'model'}],modes:[]}]})},activeRuns,runExecutor:{start:vi.fn()},roomWorkspace:workspace} as never),next=await service.execute({roomId:'demo-room',text:'next',targets:['architect']});
