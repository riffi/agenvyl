...
export class RunInterventionService{
  ...
  async create(runId:string,input:{intervention_id:string;text:string}):Promise<CreateRunInterventionResult>{
    ...
    if(!run||run.terminal||control.status!=='streaming'||run.status!=='streaming')throw new AppError('run_not_intervenable',409,'Instructions can be added only while a run is actively streaming');
    ...
    if(run.pendingIntervention){
      ...
      throw new AppError(run.pendingIntervention.id===input.intervention_id?'intervention_conflict':'intervention_in_progress',409,run.pendingIntervention.id===input.intervention_id?'Intervention ID is already used with different text':'Another instruction is already in progress');
    }
    ...
    if(!executionId)throw new AppError('run_not_intervenable',409,'The active run cannot accept an instruction');
