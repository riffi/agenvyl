...
export async function registerRunRoutes(app:FastifyInstance,runs:RunsService){
  ...
  app.post<{Params:{runId:string};Body:ResolveRunRequest}>('/api/v1/runs/:runId/approval',{schema:{params:runParamsSchema,body:runRequestResolutionBodySchema}},request=>runs.approve(request.params.runId,request.body));
  app.post<{Params:{runId:string};Body:ResolveRunRequest}>('/api/v1/runs/:runId/request',{schema:{params:runParamsSchema,body:runRequestResolutionBodySchema}},request=>runs.approve(request.params.runId,request.body));
}
