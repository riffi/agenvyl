...
export class RunExecutor {
  ...
  private async workflowInstructions(run:RunContext){
    ...
    return`\n\n<workflow_mode mode="plan" enforcement="${run.executionProfile.planEnforcement}">\nWork in read-only exploration mode. Inspect the request, the recommended project, and relevant context; discuss findings, options, risks, and tradeoffs, and ask focused clarifying questions when needed. Answer naturally. Do not produce a formal implementation plan or create plan.md unless the human explicitly asks for a plan. The workflow mode overrides imperative wording in the message: do not implement the requested change while Plan is active. Do not create, edit, move, or delete files in the recommended external project. Do not install dependencies or run commands that mutate project or external-system state.\n\nMCP operations are the only exception to the external-system restriction. Before any operation through an MCP server, including a read-only operation, obtain explicit confirmation from the human through the harness's structured clarification mechanism and wait for the answer. Built-in clarification and approval tools are not MCP operations for this rule. The confirmation request must name the integration, target, and bounded batch of planned operations, including foreseeable side effects. Imperative wording in the original task does not count as this confirmation. A positive answer authorizes only the described MCP batch in the current run; it does not switch the room or run to Work mode. Complete the confirmed batch without asking again for each call, but obtain new confirmation before a materially different MCP operation or target. If structured clarification is unavailable, confirmation is declined, or the answer is ambiguous, do not call the MCP server; provide the analysis or a draft instead.\n\nManaged artifacts may be created inside the Agenvyl run workspace when the harness permits it; normal room-workspace publication still applies.${run.executionProfile.planEnforcement==='instruction_only'?' This harness has no native Plan enforcement, so these restrictions are instruction-only and cannot technically prevent external project writes.':''}\n</workflow_mode>`;
  }
