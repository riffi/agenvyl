...
describe('RunExecutor', () => {
  ...
  it('requires structured clarification before a Plan response when the harness supports it',async()=>{
    ...
    registry.add({...run('plan-run'),executionProfile:{...workProfile,workflowMode:'plan',planEnforcement:'native'}});
  ...
  it('starts runs in FIFO order up to the concurrency limit',async()=>{
    ...
    await vi.waitFor(()=>expect(created).toEqual(['upstream-1']));await vi.waitFor(()=>expect(firstStream).toBeDefined());expect(executor.stats()).toMatchObject({active:1,queued:1,limit:1});
  ...
  it('refreshes conversation history when a queued turn actually starts and strips internal manifests',async()=>{
    ...
    await database.sql`INSERT INTO agent_runs(id,message_id,room_id,persona_id,persona_version_id,persona_handle,requested_model,harness_instance_id,harness_type,model_id,execution_profile,status,response_slot_id,context,created_at,updated_at) VALUES(${nextRun},${nextMessage},'demo-room','persona-architect','persona-architect-v1','architect','sol','local-hermes','hermes','sol',${database.sql.json(workProfile)},'queued',${nextRun},${database.sql.json([])},${next},${next})`;
  ...
  it('recovers persisted non-terminal runs idempotently without a Connector checkpoint',async()=>{const {executor,events,database}=await fixture(vi.fn<typeof fetch>());const now=new Date().toISOString(),messageId=crypto.randomUUID(),runId=crypto.randomUUID();await database.sql`INSERT INTO room_messages(id,room_id,text,targets,run_ids,created_at)VALUES(${messageId},'demo-room','orphan',${database.sql.json([])},${database.sql.json([runId])},${now})`;await database.sql`INSERT INTO agent_runs(id,message_id,room_id,persona_id,persona_version_id,persona_handle,requested_model,harness_instance_id,harness_type,model_id,execution_profile,status,upstream_run_id,created_at,updated_at)VALUES(${runId},${messageId},'demo-room','persona-architect','persona-architect-v1','architect','sol','local-hermes','hermes','sol',${database.sql.json(workProfile)},'streaming','upstream-orphan',${now},${now})`;expect(await executor.reconcilePersistedRuns()).toBe(1);expect(await executor.reconcilePersistedRuns()).toBe(0);expect((await database.sql`SELECT status FROM agent_runs WHERE id=${runId}`)[0]?.status).toBe('failed');expect((await events.replay('demo-room',0)).some(event=>event.type==='run.status'&&(event.payload as {runId?:string}).runId===runId)).toBe(true);await executor.shutdown();await database.close();});

  it('fails a persisted Connector run from an older epoch and keeps it retryable',async()=>{
    const inspect=vi.fn<ConnectorLifecycle['inspect']>(),connector:ConnectorLifecycle={health:vi.fn().mockResolvedValue({apiVersion:'v2',connectorEpoch:'epoch-new',status:'ready',startedAt:new Date().toISOString(),instances:{total:1,healthy:1,degraded:0}}),inspect};
    const {executor,events,database,personas,messages,runs}=await fixture(vi.fn<typeof fetch>(),4,connector),persona=(await personas.find('persona-architect'))!,round=await messages.createRound('demo-room','orphan',[persona],profiles([persona])),runId=round.runs[0].id;
  ...
  it('waits for a temporarily unavailable Connector before recovering persisted execution',async()=>{
    ...
    const {executor,database,personas,messages}=await fixture(vi.fn<typeof fetch>(),4,connector,transport,undefined,1),persona=(await personas.find('persona-architect'))!,round=await messages.createRound('demo-room','recover after restart',[persona],profiles([persona])),runId=round.runs[0].id;
  ...
  it('reattaches a same-epoch run and replays only events after the durable cursor',async()=>{
    ...
    const {executor,database,personas,messages}=await fixture(vi.fn<typeof fetch>(),4,connector,transport),persona=(await personas.find('persona-architect'))!,round=await messages.createRound('demo-room','active',[persona],profiles([persona])),runId=round.runs[0].id;
  ...
  it('restores and resolves a same-epoch MCP elicitation through Connector',async()=>{
    let release!:()=>void;const proceed=new Promise<void>(resolve=>{release=resolve;}),pending={id:'elicitation-recovered',kind:'elicitation' as const,prompt:'Choose workspace',elicitation:{mode:'form' as const,serverName:'nodexium',message:'Choose workspace',requestedSchema:{type:'object',properties:{workspace:{type:'string'}}}}},snapshot={...connectorContractFixtures.execution,executionId:'execution-elicitation',status:'waiting_for_user' as const,cursor:7,earliestReplayableCursor:1,pendingRequests:[pending]},connector=executionClient(snapshot,async function*(){await proceed;yield{...connectorEvent(8,'request.resolved',{requestId:pending.id,outcome:'answered'}),executionId:'execution-elicitation'};yield{...connectorEvent(9,'execution.completed',{}),executionId:'execution-elicitation'};}),transport=new ConnectorRunAdapter(connector),{executor,registry,database,personas,messages}=await fixture(vi.fn<typeof fetch>(),4,connector,transport),persona=(await personas.find('persona-architect'))!,round=await messages.createRound('demo-room','control',[persona],profiles([persona])),runId=round.runs[0].id;
    vi.mocked(connector.resolve).mockResolvedValue({execution:{...snapshot,cursor:8,pendingRequests:[]},request:pending});await database.sql`UPDATE agent_runs SET status='waiting_clarification',connector_execution_id='execution-elicitation',connector_epoch='epoch-1',connector_cursor=7 WHERE id=${runId}`;
    expect(await executor.reconcilePersistedRuns()).toBe(0);await vi.waitFor(()=>expect(registry.get(runId)?.pendingRequests?.get(pending.id)?.kind).toBe('elicitation'));await executor.approve(runId,pending.id,{elicitation:{action:'accept',content:{workspace:'main'}}});expect(connector.resolve).toHaveBeenCalledWith('execution-elicitation','elicitation-recovered',{elicitation:{action:'accept',content:{workspace:'main'}}});release();await vi.waitFor(async()=>expect((await database.sql`SELECT status FROM agent_runs WHERE id=${runId}`)[0]?.status).toBe('completed'));await executor.shutdown();await database.close();
  });

  it('leaves an active Connector run recoverable during graceful Core shutdown',async()=>{
    let markStreamAttached!:()=>void;
    const streamAttached=new Promise<void>(resolve=>{markStreamAttached=resolve;});
    const snapshot={...connectorContractFixtures.execution,cursor:2,pendingRequests:[]},connector=executionClient(snapshot,async function*(_executionId,options){await new Promise<void>((_resolve,reject)=>{options.signal.addEventListener('abort',()=>reject(new DOMException('Aborted','AbortError')),{once:true});markStreamAttached();});}),transport=new ConnectorRunAdapter(connector);vi.mocked(connector.start).mockImplementation(async request=>({...snapshot,executionId:request.executionId}));const{executor,registry,database,personas,messages}=await fixture(vi.fn<typeof fetch>(),4,connector,transport),persona=(await personas.find('persona-architect'))!,round=await messages.createRound('demo-room','shutdown',[persona],profiles([persona])),runId=round.runs[0].id;registry.add(run(runId,round.message.id));executor.start(runId,'shutdown');await vi.waitFor(async()=>expect((await database.sql`SELECT status,connector_cursor FROM agent_runs WHERE id=${runId}`)[0]).toEqual({status:'streaming',connector_cursor:'2'}));await streamAttached;expect(await executor.shutdown()).toBe(true);expect((await database.sql`SELECT status FROM agent_runs WHERE id=${runId}`)[0]?.status).toBe('streaming');await database.sql`UPDATE agent_runs SET status='failed' WHERE id=${runId}`;await database.close();
  },15_000);
  ...
  it('persists Connector identity and every accepted stream checkpoint on the opt-in execution path',async()=>{
    ...
    const transport=new ConnectorRunAdapter(connector),{executor,registry,database,personas,messages}=await fixture(vi.fn<typeof fetch>(),4,connector,transport),persona=(await personas.find('persona-architect'))!,round=await messages.createRound('demo-room','hello',[persona],profiles([persona])),runId=round.runs[0].id;
...
function connectorEvent<T extends ConnectorExecutionEvent['type']>(cursor:number,type:T,payload:Extract<ConnectorExecutionEvent,{type:T}>['payload']){return{apiVersion:'v2',connectorEpoch:'epoch-1',executionId:'run-1',cursor,occurredAt:'2026-07-17T00:00:00.000Z',type,payload} as Extract<ConnectorExecutionEvent,{type:T}>;}
