...
describe('setup API',()=>{
  ...
  it('creates Architect, Builder, and Reviewer against a verified route',async()=>{
    const request=vi.fn<typeof fetch>(async url=>{const path=new URL(String(url)).pathname;if(path==='/v1/instances')return Response.json({apiVersion:'v1',connectorEpoch:'epoch',instances:[{id:'local-opencode',type:'opencode',status:'healthy',capabilities:['model_catalog']}]});if(path==='/v1/instances/local-opencode/catalog')return Response.json({apiVersion:'v1',connectorEpoch:'epoch',instanceId:'local-opencode',models:[{id:'model-1'}],modes:[]});if(path==='/v1/discovery')return Response.json({apiVersion:'v1',candidates:[]});return new Response('{}',{status:404});});
