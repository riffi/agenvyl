...
import type {CompleteSetupRequest,ConfigureSetupHarnessesRequest,TestHarnessInstanceRequest} from '@agenvyl/contracts';
...
export async function registerSetupRoutes(app:FastifyInstance,setup:SetupService){
  ...
  app.post<{Params:{id:string}}>('/api/v1/harnesses/:id/restart',request=>setup.restartHarness(request.params.id));
