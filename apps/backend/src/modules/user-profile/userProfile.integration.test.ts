...
describe('local user profile API',()=>{
  it('reads, normalizes and atomically updates the singleton profile',async()=>{
    ...
    expect((await app.inject('/api/v1/user-profile')).json()).toMatchObject({id:'local-user',displayName:'User',handle:'user'});
  ...
  it('returns typed validation, reserved and persona-conflict errors without partial updates',async()=>{
    ...
    expect((await app.inject('/api/v1/user-profile')).json()).toMatchObject({displayName:'User',handle:'user'});
