...
export function toTimelineRun(row: DatabaseRow, tools: ToolActivity[], requests:RunRequest[]=[],interventions:RunIntervention[]=[],artifacts:RunArtifact[]=[],embeds:RunEmbed[]=[],artifactSummary?:RunArtifactSummary,staticPreview?:WorkspaceAttachment,staticPreviewStatus?:Run['staticPreviewStatus']): Run {
  ...
  return {
    ...
    ...(row.continued_from_run_id==null?{}:{continuedFromRunId:text(row.continued_from_run_id)}),
    ...(row.continuation_instruction==null?{}:{continuationInstruction:text(row.continuation_instruction)}),
    ...(row.continuation_author&&typeof row.continuation_author==='object'?{continuationAuthor:row.continuation_author as Run['continuationAuthor']}:{}),
    ...(row.continuation_retention==='explicit_release'||row.continuation_retention==='provider_managed'?{continuationRetention:row.continuation_retention}:{}),
    ...
    requests, ...(row.error == null ? {} : { error:text(row.error) }),...(row.error_code == null ? {} : { errorCode:text(row.error_code) }),artifacts,...(artifactSummary?{artifactSummary}:{}),...(staticPreview?{staticPreview}:{}),...(staticPreviewStatus?{staticPreviewStatus}:{}),embeds,
    ...
    ...(row.base_head?{workspaceResult:{base_head:text(row.base_head),...(row.result_head?{result_head:text(row.result_head)}:{}),...(row.checkpoint_sha?{checkpoint_sha:text(row.checkpoint_sha)}:{}),capture_status:text(row.workspace_capture_status) as NonNullable<Run['workspaceResult']>['capture_status'],errors:Array.isArray(row.workspace_errors)?row.workspace_errors as NonNullable<Run['workspaceResult']>['errors']:[],updated_at:timestamp(row.workspace_updated_at)}}:{}),
  };
}
