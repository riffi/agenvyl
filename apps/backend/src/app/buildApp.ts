...
export async function buildApp(options: AppOptions = {}) {
  ...
  const { database, events, dependencyHealth, runExecutor, roomsService, personasService, userProfileService, personaGroupsService, createMessageRound, runsService,roomWorkspace,harnessCatalogService } = await createAppContainer(config, options.fetch,app.log);
