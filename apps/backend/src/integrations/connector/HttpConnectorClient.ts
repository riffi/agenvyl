import {isDirectoryListing,type DirectoryListing} from '@agenvyl/contracts';
...
export class HttpConnectorClient implements ConnectorExecutionClient {
  ...
  async validateDirectory(path:string):Promise<ConnectorDirectoryValidation>{const value=await this.json('/v2/directories/validate','POST',{path},'discovery',15_000);if(!isConnectorDirectoryValidation(value))throw invalidResponse('Connector returned an invalid directory validation result');return value;}
  async browseDirectories(path?:string):Promise<DirectoryListing>{const value=await this.json('/v2/directories/browse','POST',{path},'discovery',30_000);if(!isDirectoryListing(value))throw invalidResponse('Connector returned an invalid folder listing');return value;}
  async pickDirectory():Promise<ConnectorDirectoryPickerResult>{const value=await this.json('/v2/directories/pick','POST',{},'discovery',5*60_000+5_000);if(!isConnectorDirectoryPickerResult(value))throw invalidResponse('Connector returned an invalid directory picker result');return value;}
