...
import {expandProjectReferences} from '@agenvyl/contracts';
...
export class ConnectorRunAdapter implements RunGateway,RunEventStream,RunRecovery,DependencyHealth {
  ...
  async createRun(input:StartRunInput):Promise<RunHandle>{
    const {history}=experimentalTailV2ConversationHistory((input.conversationHistory??[]).map(item=>({...item,content:expandProjectReferences(item.content)})));
    const execution=await this.connector.start({
      ...
      input:{systemPrompt:expandProjectReferences(input.instructions),history,message:expandProjectReferences(input.input),...(input.attachments?.length?{attachments:input.attachments}:{})},
  ...
  async intervene(executionId:string,intervention:{interventionId:string;text:string;attachments?:import('@agenvyl/connector-contract').ExecutionAttachmentReference[]}){
    const result=await this.connector.intervene(executionId,{...intervention,text:expandProjectReferences(intervention.text)});
