...
describe('HttpConnectorClient', () => {
  ...
  it('maps missing executions and malformed payloads to safe errors', async () => {
    ...
    const rejected=new HttpConnectorClient('http://connector.test',token,vi.fn<typeof fetch>().mockResolvedValue(Response.json({apiVersion:'v2',error:'instance_not_found',message:'missing'},{status:404})));
  ...
  it('sends typed start, stop and request-resolution commands',async()=>{
    ...
    expect(request.mock.calls.map(([url,init])=>[url,init?.method,init?.body?JSON.parse(String(init.body)):undefined])).toEqual([
      ['http://connector.test/v2/executions','POST',connectorContractFixtures.startExecution],
      ['http://connector.test/v2/executions/run-1/stop','POST',undefined],
      ['http://connector.test/v2/executions/run-1/requests/request-1/resolve','POST',{resolution:'once'}],
    ]);
  });
  ...
  it('fails closed on cursor gaps, epoch changes and unavailable replay',async()=>{
    ...
    const replay=new HttpConnectorClient('http://connector.test',token,vi.fn<typeof fetch>().mockResolvedValue(Response.json({apiVersion:'v2',error:'replay_unavailable',message:'gone'},{status:409})));
