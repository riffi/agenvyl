...
describe.sequential('Core -> Connector -> installed Antigravity live smoke', () => {
  ...
  it('covers catalog, plan text, native continuation, accept-edits workspace mutation and cancel', async () => {
    ...
    expect(instance).toMatchObject({ type: 'antigravity', status: 'healthy', capabilities: ['model_catalog','execution_profiles','text_streaming','reasoning','tools','usage'],postTurnContinuation:{mode:'native_session',durability:'connector_restart',retention:'provider_managed'} });
