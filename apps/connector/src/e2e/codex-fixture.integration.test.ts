...
describe('Codex Connector/Core fixture',()=>{
  it('maps a complete run with approval, multi-question clarification, tools, usage, replay, cancel and concurrency',async()=>{
    ...
    const first=await core.createRun({executionId:'parallel-1',harnessInstanceId:'local-codex',modelId:'fixture-model',executionProfile:{workflowMode:'work',requestedReasoningEffort:'default',reasoningEffort:'default',reasoningEffortFallback:false,planEnforcement:null,permissionProfileId:'workspace-write',agentVariantId:null,implementationPlanVersionId:null},workspace:{roomId:'room',relativePath:'.'},input:'one',sessionId:'one',instructions:'',model:'fixture-model'}),second=await core.createRun({executionId:'parallel-2',harnessInstanceId:'local-codex',modelId:'fixture-model',executionProfile:{workflowMode:'work',requestedReasoningEffort:'default',reasoningEffort:'default',reasoningEffortFallback:false,planEnforcement:null,permissionProfileId:'workspace-write',agentVariantId:null,implementationPlanVersionId:null},workspace:{roomId:'room',relativePath:'.'},input:'two',sessionId:'two',instructions:'',model:'fixture-model'});expect(first.id).not.toBe(second.id);await core.stop(second.id);expect((await client.inspect(second.id)).status).toBe('cancelled');
  });
});
