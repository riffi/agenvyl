...
describe('Codex Connector/Core fixture',()=>{
  it('maps a complete run with approval, multi-question clarification, tools, usage, replay, cancel and concurrency',async()=>{
    const root=await mkdtemp(join(tmpdir(),'agenvyl-codex-fixture-'));cleanups.push(()=>rm(root,{recursive:true,force:true}));await mkdir(join(root,'room'));
    ...
    const client=new HttpConnectorClient(`http://127.0.0.1:${address.port}`,token),core=new ConnectorRunAdapter(client);expect((await client.catalog('local-codex')).models[0]).toMatchObject({id:'fixture-model',supportedModeIds:expect.arrayContaining(['workspace-write/high'])});
    const handle=await core.createRun({executionId:'run-main',harnessInstanceId:'local-codex',modelId:'fixture-model',modeId:'workspace-write/high',workspace:{roomId:'room',relativePath:'.'},input:'Do it',sessionId:'session',instructions:'Be useful',conversationHistory:[],model:'fixture-model'}),iterator=core.stream(handle.id,'local-main',new AbortController().signal)[Symbol.asyncIterator]();
    ...
    const first=await core.createRun({executionId:'parallel-1',harnessInstanceId:'local-codex',modelId:'fixture-model',modeId:'workspace-write/default',workspace:{roomId:'room',relativePath:'.'},input:'one',sessionId:'one',instructions:'',model:'fixture-model'}),second=await core.createRun({executionId:'parallel-2',harnessInstanceId:'local-codex',modelId:'fixture-model',modeId:'workspace-write/default',workspace:{roomId:'room',relativePath:'.'},input:'two',sessionId:'two',instructions:'',model:'fixture-model'});expect(first.id).not.toBe(second.id);await core.stop(second.id);expect((await client.inspect(second.id)).status).toBe('cancelled');
  });
});
