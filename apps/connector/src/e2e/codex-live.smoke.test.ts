...
describe.runIf(live)('Codex CLI live smoke',()=>{
  ...
  it('covers catalog, exact text, tool/file change, usage and cancel',async()=>{
    const catalog=await adapter.catalog(),model=catalog.models[0];expect(model).toBeDefined();expect(model.supportedModeIds).toContain('workspace-write/default');
  ...
  const request=(id:string,message:string):AdapterStartExecutionRequest=>({executionId:id,harnessInstanceId:'local-codex',modelId:'',modeId:'workspace-write/default',workspace:{roomId:'room',relativePath:'.',absolutePath:join(root,'room')},input:{systemPrompt:'Follow the request precisely.',history:[],message}});
