...
describe.runIf(live)('Claude CLI live smoke',()=>{
  ...
  it('covers text, workspace edit, interaction and cancellation',async()=>{const catalog=await adapter.catalog(),model=catalog.models[0],effort=model?.defaultReasoningEffort??null;expect(model).toBeTruthy();const exact=await run('exact','Reply with exactly AGENVYL_CLAUDE_OK.',model!.id,'default',effort);expect(exact.text.trim()).toBe('AGENVYL_CLAUDE_OK');const file=await run('file','Create claude-live.txt containing exactly live-ok, then finish.',model!.id,'acceptEdits',effort);expect(await readFile(join(root,'room','claude-live.txt'),'utf8')).toBe('live-ok');expect(file.events.some(event=>event.type==='tool.started')).toBe(true);const cancellation=await adapter.start(request('cancel','Work until interrupted.',model!.id,'default',effort)),events=adapter.events(cancellation);await adapter.stop(cancellation);let terminal:AdapterExecutionEvent|undefined;for await(const event of events)if(['execution.cancelled','execution.completed','execution.failed'].includes(event.type))terminal=event;expect(terminal?.type).toBe('execution.cancelled');},240_000);
  const request=(id:string,message:string,modelId:string,permissionProfileId:string,reasoningEffort:string|null):AdapterStartExecutionRequest=>({executionId:id,harnessInstanceId:'local-claude',modelId,executionProfile:{workflowMode:'work',reasoningEffort,permissionProfileId,agentVariantId:null,planEnforcement:null},workspace:{roomId:'room',relativePath:'.',absolutePath:join(root,'room')},input:{systemPrompt:'Follow the request precisely.',history:[],message}});
  const run=async(id:string,message:string,model:string,permission:string,effort:string|null)=>{const execution=await adapter.start(request(id,message,model,permission,effort)),events:AdapterExecutionEvent[]=[];for await(const event of adapter.events(execution)){events.push(event);if(event.type==='request.opened')await adapter.resolveRequest(execution,event.payload.request,event.payload.request.kind==='approval'?{resolution:'once'}:{answers:Object.fromEntries((event.payload.request.questions??[]).map(question=>[question.id,[question.options?.[0]?.label??'yes']]))});}return{text:events.filter((event):event is Extract<AdapterExecutionEvent,{type:'output.text.delta'}>=>event.type==='output.text.delta').map(event=>event.payload.text).join(''),events};};
});
