...
describe('Connector config', () => {
  ...
  it('rejects Claude endpoints and the removed Codex full-access instance flag',async()=>{const endpoint=await config('version: 1\ninstances:\n  - { id: local-claude, type: claude, enabled: true, endpoint: http://127.0.0.1:1 }\n');await expect(loadConnectorConfig({path:endpoint,env:{AGENVYL_CONNECTOR_TOKEN:'x'.repeat(32)}})).rejects.toThrow('invalid for Claude');const legacy=await config('version: 1\ninstances:\n  - { id: local-codex, type: codex, enabled: true, allowDangerFullAccess: true }\n');await expect(loadConnectorConfig({path:legacy,env:{AGENVYL_CONNECTOR_TOKEN:'x'.repeat(32)}})).rejects.toThrow('unsupported keys');});
  it('accepts Cursor instances without endpoint-specific configuration',async()=>{const valid=await config('version: 1\ninstances:\n  - { id: local-cursor, type: cursor, enabled: true }\n');await expect(loadConnectorConfig({path:valid,env:{AGENVYL_CONNECTOR_TOKEN:'x'.repeat(32)}})).resolves.toMatchObject({instances:[{id:'local-cursor',type:'cursor',enabled:true}]});const endpoint=await config('version: 1\ninstances:\n  - { id: local-cursor, type: cursor, enabled: true, endpoint: http://127.0.0.1:1 }\n');await expect(loadConnectorConfig({path:endpoint,env:{AGENVYL_CONNECTOR_TOKEN:'x'.repeat(32)}})).rejects.toThrow('invalid for Cursor');});
  it('rejects the removed AGY instance permission mode',async()=>{const legacy=await config('version: 1\ninstances:\n  - { id: local-antigravity, type: antigravity, enabled: true, permissionMode: plan }\n');await expect(loadConnectorConfig({path:legacy,env:{AGENVYL_CONNECTOR_TOKEN:'x'.repeat(32)}})).rejects.toThrow('unsupported keys: permissionMode');});
