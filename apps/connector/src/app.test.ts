...
describe('Connector shell', () => {
  it('protects every v2 endpoint with Bearer auth', async () => {
    const app = buildConnectorApp(config);
    const response = await app.inject('/v2/health');
    expect(response.statusCode).toBe(401);
    expect(response.headers['www-authenticate']).toBe('Bearer');
    expect(response.json()).toEqual({ apiVersion: 'v2', error: 'unauthorized', message: 'Valid Connector Bearer token required' });
    await app.close();
  });
  ...
  it('reports an ephemeral epoch and independently degraded configured instances', async () => {
    ...
    const health = await app.inject({ url: '/v2/health', headers: auth });
  ...
  it('reports managed OpenCode ownership even when its adapter is unavailable',async()=>{
    ...
    const response=await app.inject({url:'/v2/instances',headers:auth});
  ...
  it('discovers and atomically applies bootstrap configuration behind bearer auth',async()=>{
    ...
    const app=buildConnectorApp({...config,instances:[]},{discover:async()=>({apiVersion:'v2',candidates:[]}),configureInstances:async instances=>new Map(instances.map(instance=>[instance.id,adapter])),persistInstances:async instances=>{persisted.push(structuredClone(instances));}});
    expect((await app.inject('/v2/discovery')).statusCode).toBe(401);
    expect((await app.inject({url:'/v2/discovery',headers:auth})).json()).toEqual({apiVersion:'v2',candidates:[]});
    const response=await app.inject({method:'PUT',url:'/v2/instances',headers:auth,payload:{instances:[{id:'local-hermes',type:'hermes',enabled:true,endpoint:'http://127.0.0.1:8642'}]}});
    expect(response.statusCode).toBe(200);expect(persisted).toHaveLength(1);
    expect((await app.inject({url:'/v2/instances',headers:auth})).json().instances[0]).toMatchObject({id:'local-hermes',status:'healthy'});
    await app.close();
  });
  ...
  it('fails closed until adapters implement catalog and execution lifecycle', async () => {
    ...
    expect((await app.inject({ url: '/v2/instances/local-hermes/catalog', headers: auth })).statusCode).toBe(503);
    const invalid = await app.inject({ method: 'POST', url: '/v2/executions', headers: auth, payload: {} });
    expect(invalid.statusCode).toBe(400);
    expect(invalid.json()).toMatchObject({ error: 'invalid_request' });
    const unavailable = await app.inject({ method: 'POST', url: '/v2/executions', headers: auth, payload: connectorContractFixtures.startExecution });
  ...
  it('serves a versioned catalog from a capable adapter',async()=>{
    ...
    const response=await app.inject({url:'/v2/instances/local-hermes/catalog',headers:auth});
  ...
  it('runs idempotent executions and replays ordered terminal SSE events', async () => {
    ...
    const created = await app.inject({ method: 'POST', url: '/v2/executions', headers: auth, payload: request });
    ...
    const conflict = await app.inject({ method: 'POST', url: '/v2/executions', headers: auth, payload: { ...request, modelId: 'different' } });
    ...
    const resumed = await app.inject({ url: `/v2/executions/${request.executionId}/events?after=2`, headers: auth });
  ...
  it('rejects unsafe or missing room workspaces before invoking an adapter', async () => {
    ...
    const traversal = await app.inject({
      method: 'POST', url: '/v2/executions', headers: auth,
  ...
  it('keeps cancellation terminal when an adapter completes late', async () => {
    ...
    const stopped = await app.inject({ method: 'POST', url: `/v2/executions/${request.executionId}/stop`, headers: auth });
    ...
    const inspected = await app.inject({ url: `/v2/executions/${request.executionId}`, headers: auth });
    expect(inspected.json().execution.status).toBe('cancelled');
    const stoppedAgain = await app.inject({ method: 'POST', url: `/v2/executions/${request.executionId}/stop`, headers: auth });
    expect(stoppedAgain.json().execution.status).toBe('cancelled');
    expect(adapter.stopCount).toBe(1);
    const replay = await app.inject({ url: `/v2/executions/${request.executionId}/events?after=2`, headers: auth });
  ...
  it('resolves pending requests idempotently and rejects a different second resolution', async () => {
    ...
    const waiting = await app.inject({ url: `/v2/executions/${request.executionId}`, headers: auth });
    expect(waiting.json().execution).toMatchObject({ status: 'waiting_for_user', cursor: 6, pendingRequests: [{ id: 'request-1' }] });
    const invalid = await app.inject({ method: 'POST', url: `/v2/executions/${request.executionId}/requests/request-1/resolve`, headers: auth, payload: { resolution: '' } });
    expect(invalid.statusCode).toBe(400);
    const notOffered = await app.inject({ method: 'POST', url: `/v2/executions/${request.executionId}/requests/request-1/resolve`, headers: auth, payload: { resolution: 'always' } });
    expect(notOffered.statusCode).toBe(400);
    expect(notOffered.json()).toMatchObject({ error: 'invalid_resolution' });

    const resolved = await app.inject({ method: 'POST', url: `/v2/executions/${request.executionId}/requests/request-1/resolve`, headers: auth, payload: { resolution: 'once' } });
    ...
    const repeated = await app.inject({ method: 'POST', url: `/v2/executions/${request.executionId}/requests/request-1/resolve`, headers: auth, payload: { resolution: 'once' } });
    expect(repeated.statusCode).toBe(200);
    ...
    const conflict = await app.inject({ method: 'POST', url: `/v2/executions/${request.executionId}/requests/request-1/resolve`, headers: auth, payload: { resolution: 'deny' } });
    expect(conflict.statusCode).toBe(409);
  ...
  it('accepts a custom clarification answer even when choices are suggestions', async () => {
    ...
    const resolved = await app.inject({ method: 'POST', url: `/v2/executions/${request.executionId}/requests/question-1/resolve`, headers: auth, payload: { resolution: 'WebP, please' } });
    expect(resolved.statusCode).toBe(200);
  ...
  it('keeps one bounded usage snapshot while suppressing duplicate vendor updates',async()=>{
    ...
    const inspected=await app.inject({url:`/v2/executions/${request.executionId}`,headers:auth});expect(inspected.json().execution).toMatchObject({status:'completed',usage});
    const replay=await app.inject({url:`/v2/executions/${request.executionId}/events?after=2`,headers:auth}),events=parseEvents(replay.body);expect(events.filter(event=>event.type==='usage.updated')).toHaveLength(1);expect(events.find(event=>event.type==='usage.updated')).toMatchObject({payload:{usage}});await app.close();
  });
  ...
  it('fails explicitly when a requested cursor predates the replay window', async () => {
    ...
    const lost = await app.inject({ url: `/v2/executions/${request.executionId}/events?after=0`, headers: auth });
    expect(lost.statusCode).toBe(409);
    expect(lost.json()).toMatchObject({ error: 'replay_unavailable' });
    const available = await app.inject({ url: `/v2/executions/${request.executionId}/events?after=4`, headers: auth });
...
class ControlledAdapter implements ConnectorAdapter {
  ...
  async catalog(){return{models:connectorContractFixtures.catalog.models.map(model=>({...model})),controls:{nativeWorkflowModes:[],permissionProfiles:[],agentVariants:[]}};}
...
async function waitForStatus(app: ReturnType<typeof buildConnectorApp>, executionId: string, status: string) {
  for (let attempt = 0; attempt < 50; attempt += 1) {
    const response = await app.inject({ url: `/v2/executions/${executionId}`, headers: auth });
