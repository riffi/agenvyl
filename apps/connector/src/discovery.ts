...
export async function discoverHarnesses(options:{env?:NodeJS.ProcessEnv;request?:typeof fetch;run?:typeof runVersion;codexAuth?:typeof runCodexLoginStatus}={}):Promise<ConnectorDiscovery>{
  ...
  const candidates=await Promise.all(definitions.map(async definition=>{
    ...
    const compatible=definition.type==='antigravity'&&cli.found?versionAtLeast(cli.version,'1.1.3'):definition.type==='codex'&&cli.found?versionAtLeast(cli.version,'0.145.0'):cli.found;
    ...
    const safeToSelect=definition.type==='codex'?compatible&&authorized:definition.type!=='antigravity'&&(Boolean(endpoint?.reachable)||definition.type==='opencode'&&cli.found);
    const warning=definition.type==='antigravity'?'AGY runs a subprocess per attempt and requires separate permission confirmation.':definition.type==='codex'&&!cli.found?'Install Codex CLI 0.145.0 or newer and run codex login.':definition.type==='codex'&&!compatible?'Codex CLI 0.145.0 or newer is required.':definition.type==='codex'&&!authorized?'Run codex login before enabling this connector.':undefined;
    return {type:definition.type,label:definition.label,cli:{...cli,compatible},...(endpoint?{endpoint}:{}),safeToSelect,supportsManagedServer:definition.managed,...(warning?{warning}:{})} satisfies HarnessDiscoveryCandidate;
  }));
