...
describe('buildConfiguredAdapters', () => {
  ...
  it('loads enabled Antigravity instances without an instance permission ceiling', () => {
    ...
    expect(adapters.get('local-antigravity')).toMatchObject({ type: 'antigravity', capabilities: ['model_catalog','execution_profiles','text_streaming','reasoning','tools','usage'],postTurnContinuation:{mode:'native_session',durability:'connector_restart',retention:'provider_managed'} });
