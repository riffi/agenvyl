...
class FakeAppServer implements CodexAppServerPort{
  ...
  async request(method:string,params:unknown){this.requests.push({method,params});if(method==='model/list')return{data:[{model:'codex-model',displayName:'Codex Model',hidden:false,supportedReasoningEfforts:[{reasoningEffort:'high'}]}]};if(method==='thread/start')return{thread:{id:`thread-${this.requests.filter(request=>request.method==='thread/start').length}`}};if(method==='thread/resume')return{thread:{id:(params as {threadId:string}).threadId}};if(method==='thread/delete')return{};if(method==='turn/start')return{turn:{id:`turn-${this.requests.filter(request=>request.method==='turn/start').length}`}};if(method==='turn/interrupt')return{};throw new Error(`Unexpected ${method}`);}
...
describe('Codex connector adapter',()=>{
  ...
  it('resumes an exact native thread without replay and deletes it explicitly',async()=>{
    const sourceClient=new FakeAppServer(),sourceAdapter=new CodexConnectorAdapter({client:sourceClient}),first=await sourceAdapter.start(input()),firstIterator=sourceAdapter.events(first)[Symbol.asyncIterator]();
    sourceClient.emit({method:'turn/completed',params:{threadId:'thread-1',turn:{id:'turn-1',status:'completed'}}});
    const terminal=await firstIterator.next();expect(terminal.value).toMatchObject({type:'execution.completed',payload:{continuation:{handle:expect.any(String)}}});
    if(!terminal.value||terminal.value.type!=='execution.completed'||!terminal.value.payload.continuation)throw new Error('Expected continuation handle');
    const handle=terminal.value.payload.continuation.handle,continued={...input('run-2'),input:{systemPrompt:'Be precise',history:[],message:'Only summarize the API'},continuation:{handle}};
    const resumedClient=new FakeAppServer(),adapter=new CodexConnectorAdapter({client:resumedClient}),second=await adapter.startContinuation(continued,handle),secondIterator=adapter.events(second)[Symbol.asyncIterator]();
    ...
    const changedStorageAdapter=new CodexConnectorAdapter({client:new FakeAppServer(),command:'different-codex'});
    await expect(changedStorageAdapter.startContinuation({...continued,executionId:'run-4'},handle)).rejects.toMatchObject({code:'continuation_incompatible'});
    await expect(changedStorageAdapter.releaseContinuation(handle,{instanceId:'local-codex'})).rejects.toMatchObject({code:'continuation_incompatible'});
  });
