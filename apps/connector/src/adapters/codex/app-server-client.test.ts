...
describe('Codex JSONL decoder',()=>{
  it('accepts image generation messages larger than 2 MiB by default',()=>{const line=JSON.stringify({result:'a'.repeat(3*1024*1024)});expect(new JsonLineDecoder().push(`${line}\n`)).toEqual([line]);});
