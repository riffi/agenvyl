...
describe('Claude adapter',()=>{
  it('forces native Plan permission mode over an accept-edits persona profile',async()=>{const args:string[][]=[];const adapter=new ClaudeConnectorAdapter({processFactory:options=>{args.push(options.args);return new FakeProcess();}});await adapter.start({...request,executionProfile:{...request.executionProfile,workflowMode:'plan',permissionProfileId:'accept-edits',planEnforcement:'native'}});expect(args[1]).toEqual(expect.arrayContaining(['--permission-mode','plan']));expect(args[1]).not.toContain('acceptEdits');});
  ...
  it('separates assistant messages while preserving deltas within one message',async()=>{const processes:FakeProcess[]=[];const adapter=new ClaudeConnectorAdapter({processFactory:()=>{const value=new FakeProcess();processes.push(value);return value;}});await adapter.start(request);const iterator=adapter.events({upstreamId:'run-1'})[Symbol.asyncIterator](),active=processes[1]!;active.emit({type:'stream_event',event:{type:'content_block_delta',delta:{type:'text_delta',text:'First'}}});active.emit({type:'stream_event',event:{type:'content_block_delta',delta:{type:'text_delta',text:' message.'}}});active.emit({type:'assistant',message:{content:[{type:'text',text:'First message.'},{type:'tool_use',id:'tool-1',name:'Read'}]}});active.emit({type:'user',message:{content:[{type:'tool_result',tool_use_id:'tool-1'}]}});active.emit({type:'stream_event',event:{type:'content_block_delta',delta:{type:'text_delta',text:'Second message.'}}});expect((await iterator.next()).value).toMatchObject({payload:{text:'First'}});expect((await iterator.next()).value).toMatchObject({payload:{text:' message.'}});expect((await iterator.next()).value?.type).toBe('tool.started');expect((await iterator.next()).value?.type).toBe('tool.completed');expect((await iterator.next()).value).toMatchObject({payload:{text:'\n\nSecond message.'}});});
