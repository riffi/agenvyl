...
type State={id:string;sessionId:string;status:ExecutionStatus;queue:EventQueue;process:ClaudeProcessPort;pending:Map<string,Pending>;tools:Map<string,string>;textChars:number;reasoningChars:number;hasTextOutput:boolean;textBoundaryPending:boolean;cleanup:()=>Promise<void>;terminal:boolean};
...
export class ClaudeConnectorAdapter implements ConnectorAdapter{
  ...
  async start(request:AdapterStartExecutionRequest):Promise<AdapterExecution>{
    ...
    state={id:request.executionId,sessionId:randomUUID(),status:'running',queue:new EventQueue(),process,pending:new Map(),tools:new Map(),textChars:0,reasoningChars:0,hasTextOutput:false,textBoundaryPending:false,cleanup:async()=>{await permissionRun?.close();await prompt.cleanup();},terminal:false};
  ...
  private onStreamEvent(state:State,event?:Record<string,unknown>){const delta=record(event?.delta);if(event?.type==='content_block_delta'&&typeof delta?.text==='string'){const reasoning=delta.type==='thinking_delta';if(reasoning){state.reasoningChars+=delta.text.length;state.queue.push({type:'output.reasoning.delta',payload:{text:delta.text}});return;}state.textChars+=delta.text.length;this.pushTextDelta(state,delta.text);}}
  private pushTextDelta(state:State,text:string){if(!text)return;const separated=state.textBoundaryPending&&state.hasTextOutput?`\n\n${text}`:text;state.textBoundaryPending=false;state.hasTextOutput=true;state.queue.push({type:'output.text.delta',payload:{text:separated}});}
  private onAssistant(state:State,message:ClaudeMessage){const body=record(message.message),content=body?.content,usage=normalizeUsage(body?.usage);if(usage)state.queue.push({type:'usage.updated',payload:{usage}});if(!Array.isArray(content))return;let hasText=false;for(const raw of content){const block=record(raw);if(!block)continue;if((block.type==='text'||block.type==='thinking')&&typeof block.text==='string'){const reasoning=block.type==='thinking',sent=reasoning?state.reasoningChars:state.textChars;if(block.text.length>sent){const delta=block.text.slice(sent);if(reasoning)state.queue.push({type:'output.reasoning.delta',payload:{text:delta}});else this.pushTextDelta(state,delta);}if(block.type==='text'&&block.text)hasText=true;continue;}if(block.type!=='tool_use'||typeof block.id!=='string'||typeof block.name!=='string'||state.tools.has(block.id))continue;const name=redactConnectorText(block.name,128);state.tools.set(block.id,name);state.queue.push({type:'tool.started',payload:{toolId:block.id,name,safeSummary:`Claude started ${name}`,...(block.input===undefined?{}:{safeInput:safeJson(block.input)})}});}state.textChars=0;state.reasoningChars=0;if(hasText)state.textBoundaryPending=true;}
  private onToolResults(state:State,message:ClaudeMessage){const content=record(message.message)?.content;if(!Array.isArray(content))return;for(const raw of content){const block=record(raw);if(!block||block.type!=='tool_result'||typeof block.tool_use_id!=='string')continue;const failed=block.is_error===true;state.queue.push({type:failed?'tool.failed':'tool.completed',payload:{toolId:block.tool_use_id,name:state.tools.get(block.tool_use_id)??'tool',safeSummary:redactConnectorText(String(failed?'Tool failed':'Tool completed'),500)}});}}
...
export function claudeContext(request:AdapterStartExecutionRequest){const {history}=experimentalTailV1ConversationHistory(request.input.history);return`${request.input.systemPrompt.slice(0,16_000)}\n\n<AgenvylConversationHistory>\n${JSON.stringify(history)}\n</AgenvylConversationHistory>\nTreat the history as prior room context. Respond only to the current user message.`;}
