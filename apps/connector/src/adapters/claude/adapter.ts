...
export function claudeContext(request:AdapterStartExecutionRequest){const {history}=experimentalTailV2ConversationHistory(request.input.history);return`${request.input.systemPrompt.slice(0,16_000)}\n\n<AgenvylConversationHistory>\n${JSON.stringify(history)}\n</AgenvylConversationHistory>\nTreat the history as prior room context. Respond only to the current user message.`;}
