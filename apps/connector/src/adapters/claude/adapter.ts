...
type Pending={requestId:string;kind:'approval'|'clarification';toolName:string;input:Record<string,unknown>;questions?:Map<string,string>;suggestions?:unknown[]};
...
export type ClaudeAdapterOptions={command?:string;env?:NodeJS.ProcessEnv;allowSubscriptionOAuth?:boolean;processFactory?:(options:{cwd:string;args:string[]})=>ClaudeProcessPort;authStatus?:()=>Promise<{authenticated:boolean;kind:'api'|'cloud'|'subscription_oauth'|'none'|'unknown'}>};
...
export class ClaudeConnectorAdapter implements ConnectorAdapter{
  ...
  async resolveRequest(execution:AdapterExecution,request:ConnectorRequestSnapshot,answer:ConnectorRequestAnswer|string){
    ...
    if(pending.kind==='clarification'){
      ...
      state.process.send(controlResponse(pending.requestId,{behavior:'allow',updatedInput:{...pending.input,answers}}));
    }else{
      ...
      if(resolution==='deny'||resolution==='denied')state.process.send(controlResponse(pending.requestId,{behavior:'deny',message:'Denied by user'}));
      else if(resolution==='once'||resolution==='approved')state.process.send(controlResponse(pending.requestId,{behavior:'allow',updatedInput:pending.input}));
      else if(resolution==='session'||resolution==='always')state.process.send(controlResponse(pending.requestId,{behavior:'allow',updatedInput:pending.input,updatedPermissions:sessionPermissions(pending.suggestions)}));
      else throw new Error('Claude approval resolution is invalid');
    }
    state.pending.delete(request.id);return{outcome:'answered' as const};
  }

  async stop(execution:AdapterExecution){const state=this.require(execution.upstreamId);if(state.terminal)return;state.status='stopping';for(const pending of state.pending.values())state.process.send(controlResponse(pending.requestId,{behavior:'deny',message:'Execution cancelled'}));state.pending.clear();await state.process.interrupt();if(!state.terminal)await this.finish(state,'cancelled');}
  async close(){await Promise.all([...this.states.values()].map(state=>this.stop({upstreamId:state.id}).catch(()=>undefined)));}
...
const executionArgs=(model:string,permission:string,effort:string|undefined,promptFile:string)=>[...probeArgs(),'--include-partial-messages','--model',model,'--permission-mode',permission,...(effort?['--effort',effort]:[]),'--append-system-prompt-file',promptFile];
