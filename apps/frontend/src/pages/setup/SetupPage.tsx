...
export function SetupPage(){
  ...
  const toggle=(candidate:SetupHarnessCandidate)=>{if(!selected.includes(candidate.type)&&candidate.requiresConfirmation==='claude_oauth'&&!claudeOAuthConfirmed){if(prompt('Claude subscription OAuth is experimental and may conflict with Anthropic terms for third-party products. Type CLAUDE OAUTH to continue.')!=='CLAUDE OAUTH')return;setClaudeOAuthConfirmed(true);}setSelected(value=>value.includes(candidate.type)?value.filter(item=>item!==candidate.type):[...value,candidate.type]);};
  const submit=async(event:FormEvent)=>{event.preventDefault();if(agy&&agyConfirmation!=='AGY'){setError('Type AGY to confirm');return;}setBusy(true);setError('');try{
    const instances:SetupHarnessInstance[]=(state?.candidates??[]).filter(candidate=>selected.includes(candidate.type)||(candidate.type==='antigravity'&&agy)).map(candidate=>instanceConfig(candidate,state?.instances.find(instance=>instance.type===candidate.type),claudeOAuthConfirmed));
...
export function instanceConfig(candidate:SetupHarnessCandidate,existing?:SetupState['instances'][number],claudeOAuthConfirmed=false):SetupHarnessInstance{return{id:`local-${candidate.type}`,type:candidate.type,enabled:true,...(candidate.endpoint&&candidate.type!=='codex'&&candidate.type!=='claude'?{endpoint:candidate.endpoint.url}:{}),...(candidate.type==='opencode'&&(existing?.managed===true||!candidate.endpoint?.reachable)?{managed:true}:{}),...(candidate.type==='antigravity'?{permissionMode:'plan' as const}:{}),...(candidate.type==='codex'?{allowDangerFullAccess:existing?.allowDangerFullAccess??false}:{}),...(candidate.type==='claude'?{allowSubscriptionOAuth:candidate.requiresConfirmation==='claude_oauth'&&(existing?.allowSubscriptionOAuth===true||claudeOAuthConfirmed)}:{})};}
