...
export function SetupPage(){
  ...
  return <main className={styles.shell}><form className={styles.card} onSubmit={submit}>
    ...
    <section>
      ...
      {state?.candidates.some(candidate=>candidate.type==='antigravity')&&<div className={styles.additionalConnector}><span className={styles.subsectionLabel}>Requires explicit opt-in</span><section className={styles.danger}><label className={styles.dangerChoice}><input type="checkbox" checked={agy} onChange={event=>setAgy(event.target.checked)}/><HarnessIcon type="antigravity" size="md"/><span><strong>AGY</strong> — separate subprocess with a dangerous permission flag</span></label>{agy&&<input value={agyConfirmation} onChange={event=>setAgyConfirmation(event.target.value)} placeholder="Type AGY" />}</section></div>}
    </section>
    <ConnectorOptions
      selected={selected}
