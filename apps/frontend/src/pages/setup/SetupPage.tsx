...
export function SetupPage(){
  ...
  const submit=async(event:FormEvent)=>{event.preventDefault();if(agy&&agyConfirmation!=='AGY'){setError(locale==='ru'?'Введите AGY для подтверждения':'Type AGY to confirm');return;}setBusy(true);setError('');try{
    const instances:SetupHarnessInstance[]=(state?.candidates??[]).filter(candidate=>selected.includes(candidate.type)||(candidate.type==='antigravity'&&agy)).map(candidate=>instanceConfig(candidate,state?.instances.find(instance=>instance.type===candidate.type)));
  ...
  return <main className={styles.shell}><form className={styles.card} onSubmit={submit}>
    <header><div className={styles.locale}><button type="button" onClick={()=>setLocale('en')} aria-pressed={locale==='en'}>EN</button><button type="button" onClick={()=>setLocale('ru')} aria-pressed={locale==='ru'}>RU</button></div><p className={styles.eyebrow}>Agenvyl</p><h1>{configure?(locale==='ru'?'Настройка коннекторов':'Connector settings'):(locale==='ru'?'Настройка рабочего пространства':'Workspace setup')}</h1><p>{locale==='ru'?'Выберите локальные коннекторы. Они не запускают внешние процессы без вашего решения.':'Choose local connectors. They do not start external processes without your decision.'}</p></header>
    <section><div className={styles.sectionTitle}><h2>{locale==='ru'?'Коннекторы':'Connectors'}</h2><button type="button" className={styles.link} onClick={()=>setSelected(safe)}>{locale==='ru'?'Выбрать безопасные':'Select safe'}</button></div><div className={styles.options}>{state?.candidates.filter(candidate=>candidate.type!=='antigravity').map(candidate=><Candidate key={candidate.type} candidate={candidate} checked={selected.includes(candidate.type)} onChange={()=>toggle(candidate.type)}/>)}</div></section>
    ...
    {!configure&&<section className={styles.grid}><label>{locale==='ru'?'Имя':'Display name'}<input value={name} onChange={event=>setName(event.target.value)} required/></label><label>{locale==='ru'?'Псевдоним':'Handle'}<input value={handle} onChange={event=>setHandle(event.target.value)} pattern="[a-z0-9][a-z0-9_-]*" required/></label><label className={styles.wide}>{locale==='ru'?'Корень рабочих пространств':'Workspace root'}<input value={state?.workspaceRoot??''} readOnly/></label><label className={styles.wide}>{locale==='ru'?'Первая комната':'First room'}<input value={roomTitle} onChange={event=>setRoomTitle(event.target.value)} required/></label></section>}
    {error&&<p className={styles.error} role="alert">{error}</p>}<button className={styles.primary} disabled={busy}>{busy?(locale==='ru'?'Настраиваем…':'Setting up…'):configure?(locale==='ru'?'Сохранить коннекторы':'Save connectors'):(locale==='ru'?'Создать пространство':'Create workspace')}</button><p className={styles.note}>{locale==='ru'?'Можно продолжить без коннекторов и подключить их позже.':'You can continue without connectors and add them later.'}</p>
  </form></main>;
}
