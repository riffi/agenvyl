  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.content}>
      ...
      {loading&&!state?<div className={styles.loading}><Spinner label="Loading harness configuration…"/></div>:<div className={`${styles.masterDetail} ${instanceId?styles.mobileDetail:''}`}>
        ...
        <section className={styles.detail} aria-label="Harness instance editor">{selected&&draft?<HarnessInstanceEditor saved={selected} draft={draft} saving={saving} testing={testing} testMessage={testMessage} onChange={next=>{setDraft(next);setNotice('');setTestMessage(undefined);}} onSave={()=>void saveSelected()} onCancel={()=>setDraft(asDraft(selected))} onTest={()=>void test()} onRemove={()=>void remove()} onBack={()=>navigate('/settings/harnesses')}/>:<EmptyState className={styles.noSelection} icon={<Cable/>} title={instanceId?'Harness not found':'Select a harness'} description={instanceId?'This instance no longer exists. Return to the configured list.':'Choose an instance to inspect health and edit its configuration.'}/>}</section>
      </div>}
    </section>
