...
export const HarnessSettingsPage=()=>{
  ...
  const[state,setState]=useState<HarnessSettingsState>(),[draft,setDraft]=useState<HarnessDraft>(),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[testing,setTesting]=useState(false),[restarting,setRestarting]=useState(false),[refreshing,setRefreshing]=useState(false),[addOpen,setAddOpen]=useState(false);
  ...
  const restart=async()=>{if(!selected||selected.type!=='opencode'||!selected.managed||selected.activeExecutions)return;if(!confirm(`Restart ${selected.id}? Active OpenCode server state will be replaced.`))return;setRestarting(true);setError('');setNotice('');try{await harnessesApi.restart(selected.id);const[next,catalog]=await Promise.all([harnessesApi.settings(),harnessesApi.catalog(undefined,true)]);setState(next);queryClient.setQueryData(harnessKeys.catalog,catalog);setNotice(`${selected.id} restarted with a fresh model catalog.`);}catch(issue){setError(message(issue));await read(false).catch(()=>undefined);}finally{setRestarting(false);}};
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.content}>
      ...
      {loading&&!state?<div className={styles.loading}><Spinner label="Loading harness configuration…"/></div>:<div className={`${styles.masterDetail} ${instanceId?styles.mobileDetail:''}`}>
        ...
        <section className={styles.detail} aria-label="Harness instance editor">{selected&&draft?<HarnessInstanceEditor saved={selected} draft={draft} saving={saving} testing={testing} restarting={restarting} testMessage={testMessage} onChange={next=>{setDraft(next);setNotice('');setTestMessage(undefined);}} onSave={()=>void saveSelected()} onCancel={()=>setDraft(asDraft(selected))} onTest={()=>void test()} onRestart={()=>void restart()} onRemove={()=>void remove()} onBack={()=>navigate('/settings/harnesses')}/>:<EmptyState className={styles.noSelection} icon={<Cable/>} title={instanceId?'Harness not found':'Select a harness'} description={instanceId?'This instance no longer exists. Return to the configured list.':'Choose an instance to inspect health and edit its configuration.'}/>}</section>
      </div>}
    </section>
