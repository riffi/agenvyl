...
export const HarnessInstanceCard=({instance,isNew,checking,checkMessage,onChange,onRemove,onCheck}:{instance:HarnessDraft;isNew:boolean;checking:boolean;checkMessage?:string;onChange:(next:HarnessDraft)=>void;onRemove:()=>void;onCheck:()=>void})=>{
  ...
  return <article className={`${styles.instance} ${styles[instance.status]}`}>
    ...
    <div className={styles.formGrid}>
      ...
      {instance.type!=='antigravity'&&instance.type!=='codex'&&instance.type!=='claude'&&<label className={styles.full}><span>Endpoint</span><Input type="url" placeholder={instance.type==='hermes'?'http://127.0.0.1:8642':'http://127.0.0.1:4096'} value={instance.endpoint??''} onChange={event=>update('endpoint',event.target.value)}/></label>}
      {instance.type==='opencode'&&<label className={`${styles.checkField} ${styles.full}`}><input type="checkbox" checked={Boolean(instance.managed)} onChange={event=>update('managed',event.target.checked)}/><span><strong>Managed server</strong><small>Uses an existing endpoint or starts and stops an OpenCode process when needed.</small></span></label>}
      {instance.type==='opencode'&&<label className={styles.full}><span>Allowed external directories</span><TextArea aria-label="Allowed external directories" placeholder={'One absolute path per line\nC:\\work\\trusted-repo\n/home/user/trusted-repo'} value={(instance.externalDirectoryRoots??[]).join('\n')} onChange={event=>update('externalDirectoryRoots',event.target.value.split(/\r?\n/).map(root=>root.trim()).filter(Boolean))}/><small>OpenCode may request paths under these roots. Empty means external access is denied.</small></label>}
