...
export const HarnessInstanceFields=({instance,newInstance,onChange}:{instance:HarnessDraft;newInstance:boolean;onChange:(next:HarnessDraft)=>void})=>{
  ...
  return <div className={styles.formGrid}>
    ...
    {!['antigravity','codex','claude','cursor'].includes(instance.type)&&<label className={styles.full}><span>Endpoint</span><Input type="url" placeholder={instance.type==='hermes'?'http://127.0.0.1:8642':'http://127.0.0.1:4096'} value={instance.endpoint??''} onChange={event=>update('endpoint',event.target.value)}/></label>}
