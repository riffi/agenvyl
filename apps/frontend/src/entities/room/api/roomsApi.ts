...
export const roomsApi = {
  ...
  create: (title: string, personaIds: string[],projectId:string|null=null) => apiRequest<Room>('/api/v1/rooms', { method: 'POST', body: { title, persona_ids: personaIds,project_id:projectId } }),
  rename: (roomId: string, title: string) => apiRequest<Room>(`/api/v1/rooms/${encodeURIComponent(roomId)}`, { method: 'PATCH', body: { title } }),
  assignProject:(roomId:string,projectId:string|null)=>apiRequest<Room>(`/api/v1/rooms/${encodeURIComponent(roomId)}/project`,{method:'PUT',body:{project_id:projectId}}),
