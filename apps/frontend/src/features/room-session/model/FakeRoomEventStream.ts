...
export class FakeRoomEventStream implements RoomEventStream<RoomEvent> {
  ...
  private run(messageId: string, agent: AgentHandle) { const id = `run-${++this.counter}-${agent}`; const run: Run = { id, messageId, agent, harnessInstanceId:'local-hermes',harnessType:'hermes',modelId:'fake',executionProfile:{workflowMode:'work',requestedReasoningEffort:null,reasoningEffort:null,reasoningEffortFallback:false,reasoningEffortSource:'auto',planEnforcement:null,permissionProfileId:null,agentVariantId:null},status: 'queued', text: '', tools: [],interventions:[] }; this.active.add(id); this.emit({ type: 'run.created', payload: run }); return id; }
  ...
  demo(kind: FakeEventScenario) {
    ...
    if (kind === 'approval') { this.emit({ type: 'tool.updated', payload: { runId, tool: { id: 'tool-write', name: 'write_file', detail: 'src/ws-manager.ts', status: 'progress' } } }); this.later(250, () => this.emit({ type: 'request.created', payload: { runId,requestId:'request-approval', kind: 'approval', prompt: 'Allow writing src/ws-manager.ts in the isolated workspace?' } })); }
