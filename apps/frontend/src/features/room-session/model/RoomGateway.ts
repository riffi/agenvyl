...
export class FakeRoomGateway implements RoomGateway {
  ...
  async retry(){throw new ApiError(501,'unsupported','Retry недоступен в fake-режиме')}
  async select(){throw new ApiError(501,'unsupported','Выбор попытки недоступен в fake-режиме')}
...
export class HttpRoomGateway implements RoomGateway {
  ...
  send(text:string,targets?:AgentHandle[],messageId=crypto.randomUUID(),attachmentVersionIds:string[]=[]){if(!this.roomId)return Promise.reject(new ApiError(409,'room_required','Сначала создайте комнату'));return runsApi.sendMessage(this.roomId,text,targets,messageId,attachmentVersionIds);}
