...
export interface RoomGateway { readonly mode:'real'|'fake'; subscribe(listener: (event: RoomEvent) => void): () => void; send(text: string, targets?: AgentHandle[], messageId?:string,attachmentVersionIds?:string[],routing?:import('@agenvyl/contracts').MessageRouting): Promise<Message>; resolve(runId:string,requestId:string,value:import('@agenvyl/contracts').RunRequestResolution|string):Promise<void>; intervene(runId:string,text:string,interventionId?:string):Promise<void>; cancel(runId?: string): Promise<void>; retry(runId:string):Promise<void>; select(runId:string):Promise<void>; dispose(): void }
