...
export interface RoomGateway { readonly mode:'real'|'fake'; subscribe(listener: (event: RoomEvent) => void): () => void; send(text: string, targets?: AgentHandle[], messageId?:string,attachmentVersionIds?:string[],executionIntent?:ExecutionIntent): Promise<Message>; resolve(runId: string, value: import('@agenvyl/contracts').RunRequestResolution|string): Promise<void>; cancel(runId?: string): Promise<void>; retry(runId:string):Promise<void>; select(runId:string):Promise<void>; dispose(): void }
...
export class FakeRoomGateway implements RoomGateway {
  ...
  async resolve(runId: string, value: import('@agenvyl/contracts').RunRequestResolution|string) { this.stream.resolve(runId,typeof value==='string'?value:'resolution' in value?value.resolution:'answered'); }
...
export class HttpRoomGateway implements RoomGateway {
  ...
  constructor(private readonly roomId='demo-room',initialSequence?:number,initialRuns:Run[]=[]){const connectImmediately=arguments.length<2||initialSequence!==undefined;this.stream=new WebSocketRoomEventStream<RoomEvent>(roomId,initialSequence??0,connectImmediately);for(const run of initialRuns)if(['queued','streaming','finalizing','stopping','waiting_approval','waiting_clarification'].includes(run.status))this.active.add(run.id);}
  ...
  send(text:string,targets?:AgentHandle[],messageId=crypto.randomUUID(),attachmentVersionIds:string[]=[],executionIntent?:ExecutionIntent){if(!this.roomId)return Promise.reject(new ApiError(409,'room_required','Create a room first'));return runsApi.sendMessage(this.roomId,text,targets,messageId,attachmentVersionIds,executionIntent);}
  async resolve(runId:string,value:import('@agenvyl/contracts').RunRequestResolution|string){await runsApi.resolve(runId,value);}
