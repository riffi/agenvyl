...
export function ReasoningEffortChip({participant,catalog,onChange,disabled=false}:{participant:RoomPersona;catalog?:HarnessCatalog;onChange?:(value:string|null)=>Promise<unknown>|unknown;disabled?:boolean}){
  ...
  return <span ref={root} className={styles.root}>
    <button ref={trigger} type="button" className={`${styles.chip} ${inherited?'':styles.overridden}`} disabled={disabled||saving||!onChange} aria-haspopup="menu" aria-expanded={open} title={`${inherited?'Inherited':'Room override'} · ${state.source.replace('_',' ')}`} onClick={()=>setOpen(value=>!value)}>
