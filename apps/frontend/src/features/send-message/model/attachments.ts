...
export function useRoomAttachments(roomId:string){
  ...
  const runUpload=useCallback(async(item:ComposerAttachment)=>{
    ...
    try{const result=await roomsApi.uploadFile(roomId,item.file,`Inbox/${inboxName(item.file)}`,'rename',{signal:controller.signal,onProgress:progress=>update(item.id,{progress})});if(!result.version)throw new Error('The server did not return a file version');const version=result.version;update(item.id,{status:'ready',progress:100,file:undefined,name:version.path.split('/').pop()??version.path,attachment:{version_id:version.id,entry_id:version.entry_id,path:version.path,name:version.path.split('/').pop()??version.path,size:version.size,mime_type:version.mime_type,url:version.url,preview_url:version.preview_url}});void queryClient.invalidateQueries({queryKey:['rooms',roomId,'workspace']})}
