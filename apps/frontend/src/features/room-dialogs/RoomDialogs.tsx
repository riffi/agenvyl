...
function AgentPicker({title,description,personas,groups,selected,onToggle,onClose,onSubmit,submitting,error,submitLabel}:{title:string;description:string;personas:Persona[];groups:PersonaGroup[];selected:Set<string>;onToggle:(id:string)=>void;onClose:()=>void;onSubmit:()=>void;submitting:boolean;error?:string;submitLabel:string}){return <div className={styles.root}><Dialog title={title} description={description} onClose={onClose} labelledBy="agent-picker-title" footer={<><small>Selected: {selected.size}</small><Button onClick={onClose}>Cancel</Button><Button variant="primary" onClick={onSubmit} disabled={submitting}>{submitting?'Saving…':submitLabel}</Button></>}><div className={styles.agentOptions}>{personas.length?<GroupedOptions personas={personas} groups={groups} selected={selected} onToggle={onToggle}/>:<p className="muted">No agents are available. Create one in the Agents section.</p>}</div>{error&&<Alert className={styles.alert}>{error}</Alert>}</Dialog></div>}

export function CreateRoomDialog({personas,groups,onClose,onCreated}:{personas:Persona[];groups:PersonaGroup[];onClose:()=>void;onCreated:(title:string,personaIds:string[])=>Promise<void>}){const [title,setTitle]=useState('');const [selected,setSelected]=useState(()=>new Set<string>());const [saving,setSaving]=useState(false);const [error,setError]=useState<string>();const submit=async()=>{if(!title.trim()){setError('Enter a room name.');return}setSaving(true);setError(undefined);try{await onCreated(title.trim(),[...selected]);onClose()}catch(e){setError(e instanceof Error?e.message:String(e));setSaving(false)}};return <div className={styles.root}><Dialog title="New room" description="Name the room and add the agents you need." onClose={onClose} labelledBy="create-room-title" footer={<><small>Selected: {selected.size}</small><Button onClick={onClose}>Cancel</Button><Button variant="primary" onClick={()=>void submit()} disabled={saving}>{saving?'Creating…':'Create room'}</Button></>}><label className={styles.roomTitle}>Name<Input autoFocus value={title} onChange={e=>setTitle(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void submit()}} placeholder="For example, Release 2.0"/></label><h3 className={styles.sectionTitle}>ROOM AGENTS</h3><div className={`${styles.agentOptions} ${styles.roomAgents}`}><GroupedOptions personas={personas} groups={groups} selected={selected} onToggle={id=>setSelected(current=>toggleSet(current,id))}/></div>{error&&<Alert className={styles.alert}>{error}</Alert>}</Dialog></div>}

export function RoomAgentManager({personas,groups,roomPersonas,onClose,onSave}:{personas:Persona[];groups:PersonaGroup[];roomPersonas:Persona[];onClose:()=>void;onSave:(ids:Set<string>)=>Promise<void>}){const [selected,setSelected]=useState(()=>new Set(roomPersonas.map(persona=>persona.id)));const [saving,setSaving]=useState(false);const [error,setError]=useState<string>();const submit=async()=>{setSaving(true);setError(undefined);try{await onSave(selected);onClose()}catch(e){setError(e instanceof Error?e.message:String(e));setSaving(false)}};return <AgentPicker title="Room agents" description="Only connected agents can be mentioned and respond in this room." personas={personas} groups={groups} selected={selected} onToggle={id=>setSelected(current=>toggleSet(current,id))} onClose={onClose} onSubmit={()=>void submit()} submitting={saving} error={error} submitLabel="Save agents"/>}
