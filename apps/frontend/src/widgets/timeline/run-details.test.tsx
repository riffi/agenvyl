...
describe('Timeline run details', () => {
  ...
  it('renders post-turn chapters in one card and exposes an accessible continuation retry',async()=>{
    ...
    expect(container.querySelectorAll(`.${styles['run-card']}`)).toHaveLength(1);expect(screen.getByText('Original answer')).toBeTruthy();expect(screen.getByText('Focus on the API')).toBeTruthy();expect(screen.getByText('Partial revision')).toBeTruthy();expect(screen.getByText(/retained according to the harness provider/)).toBeTruthy();
    fireEvent.click(screen.getByRole('button',{name:'Retry continuation: Coder'}));expect(retry).toHaveBeenCalledWith('child');
  });

  it('offers post-turn instruction only on the selected completed head',()=>{
    const redirect=vi.fn(),source:Run={...run,harnessInstanceId:'local-codex',harnessType:'codex',responseSlotId:'slot'},state={...initialState,hydrated:true,messages:[{id:'message-1',text:'@coder work',createdAt:'2026-07-20T12:00:00.000Z',targets:['coder' as const],runIds:['run-1'],author,addressedToAll:false}],runs:{'run-1':source},runOrder:['run-1'],selectedRuns:{slot:'run-1'}},catalog={connectorEpoch:'epoch',cache:{state:'fresh' as const,refreshedAt:'2026-07-20T00:00:00.000Z',expiresAt:'2026-07-20T01:00:00.000Z'},instances:[{id:'local-codex',type:'codex',status:'healthy' as const,capabilities:[],postTurnContinuation:{mode:'native_session' as const,durability:'connector_restart' as const,retention:'explicit_release' as const},models:[],controls:{nativeWorkflowModes:[],permissionProfiles:[],agentVariants:[]},catalogCache:{state:'fresh' as const,refreshedAt:'2026-07-20T00:00:00.000Z'}}]};
