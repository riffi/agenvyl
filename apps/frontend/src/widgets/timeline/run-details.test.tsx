...
describe('Timeline run details', () => {
  ...
  it('renders post-turn chapters in one card and exposes an accessible continuation retry',async()=>{
    ...
    expect(container.querySelectorAll(`.${styles['run-card']}`)).toHaveLength(1);expect(screen.getByText('Original answer')).toBeTruthy();expect(screen.getByText('Focus on the API')).toBeTruthy();expect(screen.getByText('Partial revision')).toBeTruthy();expect(screen.queryByText(/session history/i)).toBeNull();
