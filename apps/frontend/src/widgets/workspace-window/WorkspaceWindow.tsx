...
export const WorkspaceWindow=(props:WindowProps)=>{
  ...
  if(request?.projectReference&&(!project||project.id!==request.projectReference.projectId||project.path!==request.projectReference.root))return createPortal(<section className={styles.window} role="dialog" aria-modal="true" aria-label="Project reference unavailable" onKeyDown={event=>{if(event.key==='Escape')props.onClose();if(event.key==='Tab'){const buttons=[...event.currentTarget.querySelectorAll<HTMLButtonElement>('button')];const next=event.shiftKey?buttons.at(-1):buttons[0];const edge=event.shiftKey?buttons[0]:buttons.at(-1);if(document.activeElement===edge){event.preventDefault();next?.focus();}}}}><header className={styles.globalHeader}><strong>Project reference unavailable</strong><button autoFocus onClick={props.onClose}>Close</button></header><p>The original project may have been removed or its folder changed. This reference still points to {request.projectReference.root}/{request.projectReference.path}.</p><CopyFilePath path={`${request.projectReference.root}/${request.projectReference.path}`}/></section>,document.body);
