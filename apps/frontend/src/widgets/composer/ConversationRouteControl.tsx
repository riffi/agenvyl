...
export function ConversationRouteControl({mode,delivery,saving,applyAvailable,onModeChange,onDeliveryChange}:{mode:VisibleRoutingMode;delivery:RouteDelivery;saving:boolean;applyAvailable:boolean;onModeChange:(mode:VisibleRoutingMode)=>void;onDeliveryChange:(delivery:RouteDelivery)=>void}){
  ...
  return <div className={styles['route-control']} ref={root}>
    ...
    {open&&<section className={styles['route-popover']} role="dialog" aria-label="Message route">
      ...
      {mode!=='room_context'&&<fieldset className={styles['delivery-options']}><legend>When</legend>
        ...
        <label><input type="radio" name="route-delivery" checked={delivery==='new_request'} onChange={()=>onDeliveryChange('new_request')}/><span><strong>New request</strong><small>Use room context instead of a session.</small></span></label>
      </fieldset>}
    </section>}
  </div>;
}
