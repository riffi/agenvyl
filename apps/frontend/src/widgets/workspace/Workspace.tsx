...
export function WorkspaceApp({
  ...
  const referenceActions={project:currentRoom?.project??undefined,openWorkspaceLink:(href:string,artifacts:import('@agenvyl/contracts').WorkspaceAttachment[],opener:HTMLElement)=>openWorkspaceFileLink(roomId,href,artifacts,opener,pushWorkspace),open:(reference:ProjectReference)=>pushWorkspace({origin:'workspace',source:'project',section:'files',treeVisible:reference.kind==='directory'||!matchMedia('(max-width: 899px)').matches,projectReference:reference,mode:reference.line?'source':undefined,opener:document.activeElement instanceof HTMLElement?document.activeElement:null}),insert:(reference:ProjectReference)=>{closeWorkspace();composerRef.current?.insertProjectReference(reference);}};
