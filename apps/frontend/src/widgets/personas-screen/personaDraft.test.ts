...
const persona:Persona={id:'persona-1',handle:'architect',name:'Architect',color:'#64748b',requested_model:'sol',harness_instance_id:'local-hermes',harness_type:'hermes',model_id:'sol',permission_profile_id:null,agent_variant_id:null,default_reasoning_effort:null,system_prompt:'Проектируй',group_id:null,archived_at:null};
...
describe('persona draft',()=>{
  ...
  it('creates a draft with route permissions and agent variant, never removed persona fields',()=>{const draft=newPersonaDraft({connectorEpoch:'codex',cache,instances:[{id:'local-codex',type:'codex',status:'healthy',capabilities:[],models:[{id:'gpt-5'}],controls:controls(['workspace-write'],['reviewer']),catalogCache:{state:'fresh',refreshedAt:cache.refreshedAt}}]});expect(draft).toMatchObject({model_id:'gpt-5',permission_profile_id:'workspace-write',agent_variant_id:'reviewer'});expect(draft).not.toHaveProperty('mode_id');expect(draft).not.toHaveProperty('role')});
  ...
  it('sends canonical route fields and model alias without a persona role',()=>{
    const input=personaInputFromDraft({...persona,harness_instance_id:'local-opencode',harness_type:'opencode',model_id:'gpt-5',requested_model:'stale',agent_variant_id:'build'});
    expect(input).toMatchObject({handle:'architect',harness_instance_id:'local-opencode',model_id:'gpt-5',requested_model:'gpt-5',permission_profile_id:null,agent_variant_id:'build'});
