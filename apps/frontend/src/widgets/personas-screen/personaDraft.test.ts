...
const catalog:HarnessCatalog={connectorEpoch:'epoch-1',instances:[
  ...
  {id:'local-opencode',type:'opencode',status:'healthy',capabilities:['model_catalog'],models:[{id:'default'},{id:'gpt-5'}],controls:controls([],['build','plan'])},
]};
...
describe('persona draft',()=>{
  ...
  it('creates a draft with route permissions and agent variant, never workflow state',()=>{const draft=newPersonaDraft({connectorEpoch:'codex',instances:[{id:'local-codex',type:'codex',status:'healthy',capabilities:[],models:[{id:'gpt-5'}],controls:controls(['workspace-write'],['reviewer'])}]});expect(draft).toMatchObject({model_id:'gpt-5',permission_profile_id:'workspace-write',agent_variant_id:'reviewer'});expect(draft).not.toHaveProperty('mode_id')});
