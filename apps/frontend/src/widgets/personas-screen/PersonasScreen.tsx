...
import {DangerousPermissionDialog,type DangerousPermission} from './DangerousPermissionDialog';
...
export function PersonasScreen({
  ...
  const [dangerousPermissionConfirmation,setDangerousPermissionConfirmation]=useState<DangerousPermission>();
  ...
  useEffect(()=>setDangerousPermissionConfirmation(undefined),[selectedPersonaId]);
  ...
  const changePermissionProfile=(value:string|null)=>{
    if(form?.harness_type==='codex'&&value==='danger-full-access'&&form.permission_profile_id!=='danger-full-access'){
      setDangerousPermissionConfirmation('codex-full-access');
      return;
    }
